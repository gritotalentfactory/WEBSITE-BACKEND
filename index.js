...
app.post('/admin/talents', upload.single('image'), validateTalent, async (req, res) => {
  ...
  const { name, country, skillSet, level, gender, portfolio } = req.body;
...
app.post('/talent-request', validateTalentRequest, async (req, res) => {
  ...
  const { clientName, country, skillSet, level, gender } = req.body;
  try {
    const request = new TalentRequest(req.body);
    await request.save();
    res.status(201).json({ message: 'Talent request submitted successfully', data: { clientName, country, skillSet, level, gender } });
  } catch (error) {
